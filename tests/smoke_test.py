...
from bot.services.excel import build_report_xlsx, build_users_xlsx, extract_users_from_file, extract_users_from_text
...
def check_excel() -> None:
    ...
    txt = extract_users_from_text("@user1\nhttps://t.me/user2, 123456789\n+79001234567 мусор")
    assert txt == ["@user1", "@user2", "123456789", "+79001234567"], txt
    assert extract_users_from_text("просто слова") == []
...
def check_renders() -> None:
    ...
    iv = invite_service.InviteStats(
        total=10, processed=4, invited=3, failed=1, declined=2,
        skipped_history=5, skipped_member=1, total_units=10, stop_reason="дневной лимит 50 добавлений исчерпан",
    )
    assert "дневной лимит" in invite_service.render_invite_progress(iv)
    assert "Остановлено раньше списка" in invite_service.render_invite_summary(iv)
    ...
    assert texts.INVITE_CONFIRM.format(
        accounts=2, mode="делится", chat="чят", users=10, hist="вкл",
        delay_label="30–60 c", batch="5", daily="50",
    )
    assert texts.INVITE_DELAY_ASK and texts.INVITE_DELAY_BAD
    assert texts.GATE_STATUS.format(
        state="🟢 включены", channel="x", chats="y", url="—", passes=3, hint=texts.GATE_HINT_ON,
    )
    assert texts.GATE_MSG_DEFAULT and texts.GATE_DM_DEFAULT
    kb = keyboards.gate_settings_kb(True)
    assert any(b.callback_data == keyboards.CB_GATE_TOGGLE for row in kb.inline_keyboard for b in row)
    kb = keyboards.invite_settings_kb(1, 5, 50)
    assert any(b.callback_data == keyboards.CB_INV_HIST for row in kb.inline_keyboard for b in row)
    kb = keyboards.gate_settings_kb(True)
    assert any(b.callback_data == keyboards.CB_GATE_TEST for row in kb.inline_keyboard for b in row)
    assert any(b.callback_data == keyboards.CB_GATE_RESET for row in kb.inline_keyboard for b in row)
...
async def check_db_migration() -> None:
    ...
    # --- врата подписки: KV-настройки + пропуска ---
    assert await db.get_setting("nope") is None
    await db.set_setting("gate_channel_id", "-1001234567890")
    assert await db.get_setting("gate_channel_id") == "-1001234567890"
    await db.set_setting("gate_channel_id", "-100999")  # upsert
    assert await db.get_setting("gate_channel_id") == "-100999"
    assert await db.add_gate_pass(-100999, 111) is True
    assert await db.add_gate_pass(-100999, 111) is False  # уже был
    assert await db.has_gate_pass(-100999, 111) is True
    assert await db.has_gate_pass(-100999, 222) is False
    assert await db.gate_pass_count(-100999) == 1
    assert await db.clear_gate_pass(-100999) == 1
    assert await db.gate_pass_count(-100999) == 0

    # --- врата: JSON-список чатов + загрузка настроек gate ---
    await db.set_setting("gate_chats", '{"-100555": "Чят"}')
    from bot.handlers import gate as gate_mod
    gs = await gate_mod._load(db)
    assert gs.channel_id == -100999 and gs.chats == {-100555: "Чят"}, (gs.channel_id, gs.chats)

    # --- кэш профилей (access_hash) ---
    assert await db.get_entity_hashes(7, [1, 2, 3]) == {}
    await db.save_entities(7, [(555, 111222333), (556, 444), (0, 1), (557, None)])
    got = await db.get_entity_hashes(7, [555, 556, 999])
    assert got == {555: 111222333, 556: 444}, got
    await db.save_entities(7, [(555, 777888)])  # upsert по свежему парсингу
    assert (await db.get_entity_hashes(7, [555]))[555] == 777888
    assert await db.get_entity_hashes(8, [555]) == {}  # другой владелец не видит чужой кэш
