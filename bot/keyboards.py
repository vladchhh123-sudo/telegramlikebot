...
CB_LOGIN_QR = "login:qr"             # 📷 вход по QR-коду
CB_LOGIN_PHONE = "login:phone"       # 📱 вход по номеру
...
CB_INV_DELAY_CUSTOM = "inv:delayc"   # ✍️ своя пауза
CB_INV_BATCH = "inv:batch:"          # + размер пачки (0 = без пачек)
CB_INV_DAILY = "inv:daily:"          # + дневной лимит (0 = ∞)
CB_INV_RUN = "inv:run"
CB_INV_HIST = "inv:hist"             # ⏭ скип уже пробованных вкл/выкл

# врата подписки
CB_GATE_OPEN = "gate:open"
CB_GATE_TOGGLE = "gate:toggle"
CB_GATE_SET_CHANNEL = "gate:setchan"
CB_GATE_SET_CHAT = "gate:setchat"
CB_GATE_SET_TEXT = "gate:settext"
CB_GATE_SET_DM = "gate:setdm"
CB_GATE_SET_URL = "gate:seturl"
CB_GATE_TEST = "gate:test"
CB_GATE_RESET = "gate:reset"
# сообщения
...
def main_menu(has_account: bool) -> InlineKeyboardMarkup:
    ...
    if has_account:
        ...
        rows.append([InlineKeyboardButton(text="🚪 Врата подписки", callback_data=CB_GATE_OPEN)])
    else:
...
def login_choice_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📷 Вход по QR-коду", callback_data=CB_LOGIN_QR)],
            [InlineKeyboardButton(text="📱 Вход по номеру телефона", callback_data=CB_LOGIN_PHONE)],
            [InlineKeyboardButton(text="❌ Отмена", callback_data=CB_LOGIN_CANCEL)],
        ]
    )


def login_qr_kb(url: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✅ Разрешить вход", url=url)],
            [InlineKeyboardButton(text="❌ Отмена", callback_data=CB_LOGIN_CANCEL)],
        ]
    )
...
def gate_settings_kb(enabled: bool) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🟢 Включены — выключить" if enabled else "🔴 Выключены — включить",
                    callback_data=CB_GATE_TOGGLE,
                )
            ],
            [InlineKeyboardButton(text="📢 Указать канал", callback_data=CB_GATE_SET_CHANNEL)],
            [InlineKeyboardButton(text="💬 Указать чат", callback_data=CB_GATE_SET_CHAT)],
            [
                InlineKeyboardButton(text="✍️ Текст в чате", callback_data=CB_GATE_SET_TEXT),
                InlineKeyboardButton(text="💌 Текст-подтверждение", callback_data=CB_GATE_SET_DM),
            ],
            [InlineKeyboardButton(text="🔗 Ссылка-кнопка", callback_data=CB_GATE_SET_URL)],
            [
                InlineKeyboardButton(text="🧪 Проверить настройку", callback_data=CB_GATE_TEST),
                InlineKeyboardButton(text="🧹 Сбросить пропуски", callback_data=CB_GATE_RESET),
            ],
...
INVITE_DELAY_PRESETS = [(5, 10), (15, 30), (30, 60), (60, 120), (120, 240), (300, 600)]
INVITE_BATCH_CHOICES = [3, 5, 10, 20, 0]
INVITE_DAILY_CHOICES = [20, 50, 100, 200, 0]


def invite_settings_kb(
    delay_idx: int, batch: int, daily: int, custom_label: object = None,
    skip_history: bool = True,
) -> InlineKeyboardMarkup:
    if custom_label:
        delay_text = "⏱ Пауза: " + (
            f"{int(custom_label[0])}–{int(custom_label[1])} c (своя)"
            if isinstance(custom_label, tuple)
            else str(custom_label)
        )
    else:
        idx = max(0, min(delay_idx, len(INVITE_DELAY_PRESETS) - 1))
        lo, hi = INVITE_DELAY_PRESETS[idx]
        delay_text = f"⏱ Пауза: {lo}–{hi} c"
    batch_label = "📦 Пачка: " + ("выкл" if batch == 0 else str(batch))
    daily_label = "🚦 Лимит/день: " + ("♾" if daily == 0 else str(daily))
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=delay_text, callback_data=CB_INV_DELAY + "0")],
            [InlineKeyboardButton(text="✍️ Своя пауза", callback_data=CB_INV_DELAY_CUSTOM)],
            [
            ...
            [InlineKeyboardButton(
                text="⏭ Скип пробованных: " + ("вкл" if skip_history else "выкл"),
                callback_data=CB_INV_HIST,
            )],
