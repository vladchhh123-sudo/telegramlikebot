...
async def cb_account_logout_yes(cb: CallbackQuery, manager: ClientManager, db: Database) -> None:
    try:
    ...
    if info is not None:
        ...
        row = await db.get_account(cb.from_user.id, acc_id)
        name = (row or {}).get("account_name") or f"аккаунт {acc_id}"
...
async def cb_account_logout(cb: CallbackQuery, manager: ClientManager, db: Database) -> None:
    try:
    ...
    if info is not None:
        ...
        # аккаунт вылетел (сессия убита) — имя из БД и всё равно даём выйти
        row = await db.get_account(cb.from_user.id, acc_id)
        if row is None:
            infos = await manager.list_account_infos(cb.from_user.id)
            await safe_edit(cb, texts.ACCOUNTS_HEADER.format(n=len(infos)),
                            keyboards.accounts_manage_kb(infos))
            await cb.answer("Аккаунт уже удалён", show_alert=True)
            return
        name = row.get("account_name") or f"аккаунт {acc_id}"
    await safe_edit(
        cb,
        texts.LOGOUT_CONFIRM.format(name=texts.esc(str(name))),
