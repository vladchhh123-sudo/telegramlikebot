...
def _split(items: list, parts: int) -> list[list]:
    """Распределяет чаты по аккаунтам.

    Аккаунтов больше, чем чатов → чаты зацикливаются, чтобы КАЖДЫЙ аккаунт
    получил работу (один чат парсят несколько аккаунтов — это нормально:
    у разных аккаунтов разная видимость участников и разные лимиты).
    """
    if items and parts > len(items):
        return [[items[i % len(items)]] for i in range(parts)]
...
@router.callback_query(
    F.data.startswith(keyboards.CB_PARSE_SEEN)
    | F.data.startswith(keyboards.CB_PARSE_PHOTO)
    | F.data.startswith(keyboards.CB_PARSE_USERNAME)
    | F.data.startswith(keyboards.CB_PARSE_PEERS)
    | F.data.startswith(keyboards.CB_PARSE_SCAN)
)
async def parse_stale_screen(cb: CallbackQuery, state: FSMContext) -> None:
    """Кнопки со старого экрана (состояние сбросилось после перезапуска) больше не молчат."""
            ...
            if stats.raw_users:
                with suppress(Exception):
                    await db.save_entities(
                        owner_id,
                        [(u.id, u.access_hash) for u in stats.raw_users
...
async def parse_run(
    ...
    if len(chosen) > len(chats):
        with suppress(Exception):
            await bot.send_message(
                uid,
                texts.PARSE_SPLIT_NOTE.format(accounts=len(chosen), chats=len(chats)),
            )
